        ...
        if (isset($_GET['MaMH'])) {
            ...
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    ...
                    echo "<p class='new-price'><span>$formatted_price VND</span></p>";
                    echo "<strong>- Giá đã bao gồm thuế</strong> <br/>";
                    echo "<strong>- Phí vận chuyển và các chi phí khác (nếu có) sẽ được thể hiện khi đặt hàng.</strong>";
                    echo "</div>";
                    ...
                    foreach ($_SESSION['cart'] as $key) {
                        if($key['MaMH']==$_GET['MaMH']){
                            if($key['quantity']==$SoLuongTon){
                                echo'<p class="pl-2">HẾT SẢN PHẨM!</p>';
                            }else{
                                echo'
                        ...
                        <input type="number" name="price" value="'.$formatted_price.'" hidden>
                        <input type="text" name="img" value="'.$HINH.'" hidden>
                        <button type="submit" name="cart" class="btn">
                            Thêm vào giỏ hàng <i class="fas fa-shopping-cart shopping-icon"></i>
                        </button>
                        </div>
                    </div>
                    </form>';
                    ...
                    if($check){
                        if($SoLuongTon>0){
                            ...
                            echo'<span class="pl-2" style="color:red">HẾT SẢN PHẨM!</span> ';
                        }
                    }
                }else{
                    if($SoLuongTon>0){
                        echo'
                        <strong class ="pl-2">Còn '.$SoLuongTon.' sản phẩm trong kho</strong> 
                        <br>
                        <div class="d-flex mt-3">
                        <input type="number" name="quantity" min="1" max="'.$SoLuongTon.'" value="1"/>';
                        echo'
